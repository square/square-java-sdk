...
| `CustomerId` | `String` | Optional | The [Customer](entity:Customer) ID of the customer associated with the payment.<br><br>This is required if the `source_id` refers to a card on file created using the Cards API. | String getCustomerId() |
...
| `TeamMemberId` | `String` | Optional | An optional [TeamMember](entity:TeamMember) ID to associate with<br>this payment. | String getTeamMemberId() |
...
| `BuyerPhoneNumber` | `String` | Optional | The buyer's phone number.<br>Must follow the following format:<br><br>1. A leading + symbol (followed by a country code)<br>2. The phone number can contain spaces and the special characters `(` , `)` , `-` , and `.`.<br>   Alphabetical characters aren't allowed.<br>3. The phone number must contain between 9 and 16 digits. | String getBuyerPhoneNumber() |
