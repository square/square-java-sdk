...
| `GiftCards` | [`List<GiftCard>`](/doc/models/gift-card.md) | Optional | Gift cards retrieved. | List<GiftCard> getGiftCards() |
| `Cursor` | `String` | Optional | When a response is truncated, it includes a cursor that you can use in a<br>subsequent request to fetch the next set of gift cards. If empty, this is<br>the final response. | String getCursor() |
