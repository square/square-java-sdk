/**
 * This file was auto-generated by Fern from our API Definition.
 */
package com.squareup.square.types;

import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonValue;

public final class ChangeTiming {
    public static final ChangeTiming END_OF_BILLING_CYCLE =
            new ChangeTiming(Value.END_OF_BILLING_CYCLE, "END_OF_BILLING_CYCLE");

    public static final ChangeTiming IMMEDIATE = new ChangeTiming(Value.IMMEDIATE, "IMMEDIATE");

    private final Value value;

    private final String string;

    ChangeTiming(Value value, String string) {
        this.value = value;
        this.string = string;
    }

    public Value getEnumValue() {
        return value;
    }

    @java.lang.Override
    @JsonValue
    public String toString() {
        return this.string;
    }

    @java.lang.Override
    public boolean equals(Object other) {
        return (this == other) || (other instanceof ChangeTiming && this.string.equals(((ChangeTiming) other).string));
    }

    @java.lang.Override
    public int hashCode() {
        return this.string.hashCode();
    }

    public <T> T visit(Visitor<T> visitor) {
        switch (value) {
            case END_OF_BILLING_CYCLE:
                return visitor.visitEndOfBillingCycle();
            case IMMEDIATE:
                return visitor.visitImmediate();
            case UNKNOWN:
            default:
                return visitor.visitUnknown(string);
        }
    }

    @JsonCreator(mode = JsonCreator.Mode.DELEGATING)
    public static ChangeTiming valueOf(String value) {
        switch (value) {
            case "END_OF_BILLING_CYCLE":
                return END_OF_BILLING_CYCLE;
            case "IMMEDIATE":
                return IMMEDIATE;
            default:
                return new ChangeTiming(Value.UNKNOWN, value);
        }
    }

    public enum Value {
        IMMEDIATE,

        END_OF_BILLING_CYCLE,

        UNKNOWN
    }

    public interface Visitor<T> {
        T visitImmediate();

        T visitEndOfBillingCycle();

        T visitUnknown(String unknownType);
    }
}
